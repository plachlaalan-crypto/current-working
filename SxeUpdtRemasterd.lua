...
Keybinds = {
    ...
    ["Ragdoll Self"]="NONE",["Drop Brainrot"]="R",Float="Z",Reset="X",["Auto Buy"]="K",
    ["Click to AP"]="NONE",
}
...
function instantReset()
    ...
    task.spawn(function()
        local t0 = os.clock()
        -- a remote that already killed us once this session is ALWAYS preferred
        if _G.MynxxGoodResetRemote and _G.MynxxGoodResetRemote.Parent then
            resetRemote = _G.MynxxGoodResetRemote
        end
        -- if the reset remote hasn't been captured yet, wait briefly for it
        while not resetRemote and (os.clock() - t0) < 3 do task.wait() end
        -- VALIDATED SPAM: try ONE candidate at a time for 1.5s. Not dead? That
        -- remote is the wrong one -- discard it and let the hook capture the
        -- NEXT RE/* the game fires, then try that one. Never fires several
        -- remotes blindly (that's what caused the Trade Plaza teleports).
        local tried = {}
        while lp.Character == oldChar and (os.clock() - t0) < 10 do
            local cand = resetRemote
            if not (cand and cand.Parent) or tried[cand] then
                -- pull the next untried candidate from the passive pool instead
                -- of waiting for the game to fire a new RE/* on its own
                cand = nil
                local q = _G.__MynxxResetCandidates
                if q then
                    for _, r in ipairs(q) do
                        if r.Parent and not tried[r] then cand = r; break end
                    end
                end
            end
            if cand and cand.Parent and not tried[cand] then
                local c0 = os.clock()
                while lp.Character == oldChar and (os.clock() - c0) < 1.5 do
                    pcall(function() cand:FireServer("randomstring") end)
                    task.wait()
                end
                if lp.Character ~= oldChar then
                    -- this one works: remember it for every future reset
                    _G.MynxxGoodResetRemote = cand
                    resetRemote = cand
                    break
                end
                tried[cand] = true
                resetRemote = nil -- hook will capture the next RE/* fired
            else
                task.wait(0.1)
            end
        end
...
_mynxxResetBindable = Instance.new("BindableEvent")
_mynxxResetBindable.Event:Connect(function() pcall(instantReset) end)
task.spawn(function()
    for _ = 1, 12 do
        local ok = pcall(function()
            game:GetService("StarterGui"):SetCore("ResetButtonCallback", _mynxxResetBindable)
        end)
...
function runAutoSnipe()
    ...
    -- (Old Grabble TP branch removed - now handled by Mynxx Clone-TP router above.)
    _G._isTpMoving = true
...
function findBestAnimalModel(petData)
    ...
    pcall(function()
        ...
        for _, obj in ipairs(plot:GetDescendants()) do
            ...
            if (root.Position - spawnPos).Magnitude > 12 then continue end
            local score = -(root.Position - spawnPos).Magnitude
            ...
            -- Conta partes visiveis de verdade (MeshPart com malha e nao totalmente transparente).
            -- O rig invisivel usado so pra deteccao (hitbox) normalmente tem Humanoid mas
            -- nao tem malha visivel -- por isso NAO damos bonus por Humanoid aqui.
            local visibleMeshParts = 0
            for _, d in ipairs(obj:GetDescendants()) do
                if d:IsA("MeshPart") and d.Transparency < 1 then visibleMeshParts += 1
                elseif d:IsA("BasePart") and d.Transparency < 1 and not d:IsA("MeshPart") then visibleMeshParts += 0.5 end
            end
            if visibleMeshParts > 0 then score += math.min(visibleMeshParts * 10, 100)
            else score -= 200 end -- provavelmente e' um hitbox/proxy invisivel, nao o modelo visual
...
-- MINI ICONES 3D: um mini-viewport ao vivo por linha da lista (estilo "SXE HUB PRIVAT"),
-- com cache pra nao reclonar o modelo toda hora que a lista atualiza (0.4s).
local _miniVPCache = {} -- [uid] = {frame=ViewportFrame, clone=Model, center=Vector3, radius=number, frontYaw=number, oscT=number, camera=Camera}
local function _miniBuildEntry(pet)
    ...
    corner(frame, 6)
    local cam = Instance.new("Camera"); cam.FieldOfView = 50; cam.Parent = frame; frame.CurrentCamera = cam
    local entry = {frame=frame, clone=nil, center=Vector3.new(0,0,0), radius=2.5, frontYaw=0, oscT=math.random()*10, camera=cam}
    local sourceModel = findBestAnimalModel(pet)
    if sourceModel then
        pcall(function()
            local clone = sourceModel:Clone()
            for _,desc in ipairs(clone:GetDescendants()) do
                if desc:IsA("Script") or desc:IsA("LocalScript") or desc:IsA("ModuleScript") or desc:IsA("Sound") or desc:IsA("SoundGroup") or desc:IsA("BodyMover") or desc:IsA("Constraint") then desc:Destroy()
                elseif desc:IsA("BasePart") then desc.Anchored=true; desc.CanCollide=false
                elseif desc:IsA("Animator") then for _,tr in ipairs(desc:GetPlayingAnimationTracks()) do tr:Stop(0) end end
            end
            -- Direcao para onde o modelo esta olhando (frente dele), pra camera ficar
            -- de frente em vez de mostrar as costas
...
-- Um unico Heartbeat mantem todos os mini-viewports ativos de frente pro bicho
-- (balanco leve em vez de girar 360 e mostrar as costas)
RunService.Heartbeat:Connect(function(dt)
    for uid, entry in pairs(_miniVPCache) do
        if entry.clone and entry.clone.Parent and entry.frame.Parent then
            ...
            pcall(function()
                entry.camera.CFrame = CFrame.new(entry.center + Vector3.new(math.sin(r)*dist, entry.radius*0.35, math.cos(r)*dist), entry.center)
            end)
        end
    end
end)
-- Pega (ou cria) o mini-viewport de um pet e devolve o Frame pronto pra encaixar na linha
local function getMiniIconFor(pet)
    local uid = tostring(pet.uid)
    local entry = _miniVPCache[uid]
    if not entry then
        entry = _miniBuildEntry(pet)
        _miniVPCache[uid] = entry
    end
    return entry.frame
end
-- Remove do cache os mini-viewports de pets que sumiram da lista (evita vazamento de memoria)
...
function refreshTargetPanel()
    -- Desanexa (nao destroi) os mini-viewports em cache antes do clearBody,
    -- senao o clearBody destruiria eles junto com as linhas antigas.
    ...
    local function makeTargetRow(pet, displayIndex, isPriority)
        ...
        -- Mini icone 3D ao vivo do bicho (estilo lista com icone por linha)
    ...
    local idx = 0
    -- Auto-update viewer: show selected or first priority pet
    do
        local viewerPet = SharedState and SharedState.SelectedPetData
        if not viewerPet and prioPets[1] then viewerPet = prioPets[1]
        elseif not viewerPet and otherPets[1] then viewerPet = otherPets[1] end
        if viewerPet and _G._MynxxUpdateStViewer then pcall(_G._MynxxUpdateStViewer, viewerPet) end
    end
    local seenUIDs = {}
    for _, pet in ipairs(prioPets) do idx = idx + 1; makeTargetRow(pet, idx, true); seenUIDs[tostring(pet.uid)] = true end
    ...
    for _, pet in ipairs(otherPets) do idx = idx + 1; makeTargetRow(pet, idx, false); seenUIDs[tostring(pet.uid)] = true end
...
function loadTab(tabName)
    ...
    if tabName=="Keybinds" then
        ...
        for _,name in ipairs({"Ragdoll Self","Float","Reset","Auto Buy","Click to AP"}) do makeKeybindRow(mainBody,name) end
...
dc=Instance.new("TextLabel"); dc.Name="BrandDiscord"; dc.LayoutOrder=3; dc.AutomaticSize=Enum.AutomaticSize.X; dc.Size=UDim2.new(0,0,1,0); dc.BackgroundTransparency=1; dc.Text="discord.gg/sxehubremasterd"; dc.TextColor3=Theme.Dim; dc.Font=Enum.Font.GothamBold; dc.TextSize=13; dc.TextXAlignment=Enum.TextXAlignment.Left; dc.Parent=topInfo
...
-- ============================================================
-- ESPERA O FPS ESTABILIZAR ANTES DO TP
-- ============================================================
-- ============================================================
-- ESPERA RÁPIDA PARA FPS ESTABILIZAR (máximo 1 segundo)
-- ============================================================
local function waitForStableFPS(minFPS, maxWait)
    minFPS = minFPS or 20
    maxWait = maxWait or 1.0
    local t0 = os.clock()
    while os.clock() - t0 < maxWait do
        if (_G.currentFPS or 0) >= minFPS then
            return true
        end
        task.wait(0.01)
    end
...
if Config.TpSettings.TpOnLoad then task.spawn(function()
    -- Aguarda o FPS estabilizar
    waitForStableFPS(0.1)

    -- === DELAY CONFIGURÁVEL ===
    local delay = Config.TpSettings.TpOnLoadDelay or 3
    if delay > 0 then
        task.wait(delay)  -- Aguarda o tempo definido pelo usuário
    end
...
do
    ...
    local function doVelocityTP()
        ...
        -- Respect MynxxHub manual selection, then fall back to priority/mode
        local pet
        if manuallySelectedUID then
        ...
        -- ARRIVAL GUARD (no retry): velMoveThrough can end EARLY (stall on an
        -- obstacle, flight timeout, anti-cheat lagback, death/reset). NO re-fly
        -- attempts: verify the real position below, and if we're short, abort
        -- WITHOUT cloning. A failed TP just fails -- nothing retries it.
        do
            local _rc = LP.Character
            hrp = _rc and _rc:FindFirstChild("HumanoidRootPart")
            hum = _rc and _rc:FindFirstChildOfClass("Humanoid")
            if _diedDuringTP or not hrp or not hum or (hrp.Position - destPos).Magnitude > 15 then
                if _diedConn then _diedConn:Disconnect() end
                healConn:Disconnect()
        ...
        if _cloneOk then
            -- POLL for the server-side teleport instead of a single 0.3s check:
            -- with ping/server load the clone TP often lands AFTER 0.3s, which
            -- silently skipped goToBrainrot. Success = near a plot (wider box,
            -- bases are ~50 studs across) OR clearly moved from the pre-clone spot.
            local _cloneSucceeded = false
            local _t0 = os.clock()
            local plotsFolder = workspace:FindFirstChild("Plots")
            while not _cloneSucceeded and (os.clock() - _t0) < 2 do
                task.wait(0.05)
                if _diedDuringTP then break end
                ...
                if _h and _h.Parent then
                    local p = _h.Position
                    if (p - _clonePos).Magnitude > 40 then
                        _cloneSucceeded = true
                    elseif plotsFolder then
                        local _rad = (petPos.Y <= 8.9) and 34 or 33
                        for _, plot in ipairs(plotsFolder:GetChildren()) do
                            pcall(function()
                                local pp = plot:GetPivot().Position
            ...
            -- death during/after the clone = NOT a successful clone TP (the
            -- respawn move faked the ">40 studs" success signal) -> no retry
            if _cloneSucceeded and not _diedDuringTP then goToBrainrot(petPos) end
        else
            if _caConn then _caConn:Disconnect() end
        end
        if _diedConn then _diedConn:Disconnect() end
